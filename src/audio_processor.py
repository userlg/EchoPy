...
from utils import CavaFilter, logger
...
class AudioProcessor(QObject):
    ...
    def _start_pyaudiowpatch(self) -> bool:
        ...
        try:
            ...
            for i in range(self.pa_instance.get_host_api_count()):
                if (
                    self.pa_instance.get_host_api_info_by_index(i)["type"]
                    == pyaudio.paWASAPI
                ):
                    wasapi_api_idx = i
                    break
            ...
            try:
                ...
                loopback = self.pa_instance.get_wasapi_loopback_analogue_by_dict(
                    default_speakers
                )
                ...
                for i in range(self.pa_instance.get_device_count()):
                    ...
                    if (
                        dev["hostApi"] == wasapi_api_idx
                        and "SyncMaster" in dev["name"]
            ...
            logger.info(
                f"PYAUDIO ENGINE ONLINE: {loopback['name']} ({loopback['maxInputChannels']}ch @ {self.sample_rate}Hz)"
            )
            return True
    ...
    def _pyaudio_callback(self, in_data, frame_count, time_info, status):
        ...
        channels = self.pa_instance.get_device_info_by_index(self.device_index)[
            "maxInputChannels"
        ]
    ...
    def _start_sounddevice(self, device_index: Optional[int], use_loopback: bool):
        ...
        for current_idx in device_list:
            try:
                ...
                is_wasapi_loopback = (
                    host_api == "Windows WASAPI" and hw_input_chans == 0
                )
                ...
                chan_candidates = [
                    hw_output_chans if is_wasapi_loopback else hw_input_chans,
                    2,
                    1,
                ]
                chan_candidates = sorted(
                    list(set(c for c in chan_candidates if c > 0)), reverse=True
                )
                ...
                for chans in chan_candidates:
                    ...
                    for rate in sample_rates:
                        try:
                            extra_settings = (
                                self._get_wasapi_settings()
                                if is_wasapi_loopback
                                else None
                            )

                            logger.info(
                                f"Attempting: {device_info['name']} | {rate}Hz | {chans}ch"
                            )
    ...
    def _find_mme_version(self, target_name: str) -> Optional[int]:
        ...
        try:
            ...
            mme_idx = [
                i for i, h in enumerate(sd.query_hostapis()) if h["name"] == "MME"
            ][0]
            for i, d in enumerate(devices):
                if (
                    d["hostapi"] == mme_idx
                    and target_name.split("(")[0].strip() in d["name"]
                ):
        ...
        try:
            if status:
                ...
                if status.input_overflow:
                    # Input overflow is transient in MME/DirectSound - continue processing
                    pass
            ...
            if channels == 2:
                ...
                if channels >= 3:
                    weights[2] = 0.7  # Center
                if channels >= 6:
                    weights[4:6] = 0.7  # Surround
                weights /= np.sum(weights) / 2.0
                audio_data = np.dot(indata, weights)

            # --- ULTRA-FAST NOISE FLOOR & ENERGY ---
            # Lower noise floor to be more sensitive to quiet audio
            NOISE_FLOOR = 0.00002
            # RMS calculation (Vectorized)
            activity_level = np.sqrt(
                np.mean(np.square(np.maximum(0.0, np.abs(audio_data) - NOISE_FLOOR)))
            )

            # Debug: Log first few activity levels
            if not hasattr(self, "_audio_debug_logged"):
                logger.debug(
                    f"AUDIO DEBUG: First activity_level: {activity_level:.10f}"
                )
                logger.debug(
                    f"AUDIO DEBUG: audio_data RMS: {np.sqrt(np.mean(np.square(audio_data))):.10f}"
                )
                logger.debug(
                    f"AUDIO DEBUG: audio_data max: {np.max(np.abs(audio_data)):.10f}"
                )
            ...
            activity_level = activity_level * user_gain_factor * 10000.0
    ...
    def get_devices(self) -> List[dict]:
        ...
        try:
            ...
            if not input_devices:
                logger.warning(
                    "No specific System Audio devices found. Listing all non-microphone input devices as fallback."
                )
                for i, device in enumerate(devices):
                    device_name = device["name"].lower()
                    if device["max_input_channels"] > 0 and not any(
                        kw in device_name for kw in exclude_keywords
                    ):
                        input_devices.append(
                            {
                                "index": i,
        ...
        try:
            ...
            try:
                ...
                for i, d in enumerate(sd.query_devices()):
                    is_wasapi = (
                        "Windows WASAPI" in sd.query_hostapis(d["hostapi"])["name"]
                    )
                    # Primary target: WASAPI version of the default output or SyncMaster
                    if is_wasapi and (
                        default_out_name in d["name"]
                        or "syncmaster" in d["name"].lower()
                    ):
    ...
    def set_device(self, device_index: Optional[int]):
        ...
        if device_index == self.device_index and self.is_running:
            logger.info(
                f"Device change ignored: Already on device index {device_index}"
            )
            return
