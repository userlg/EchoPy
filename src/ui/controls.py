...
class ControlPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        arrow_icon = get_resource_path(
            os.path.join("resources", "icons", "arrow_down.svg")
        ).replace("\\", "/")
        ...
        for i, theme_name in enumerate(themes):
            ...
            btn.clicked.connect(
                lambda checked, name=theme_name.lower(): self._on_theme_clicked(name)
            )
        ...
        icon_path = get_resource_path(
            os.path.join("resources", "icons", "settings.svg")
        )
        if os.path.exists(icon_path):
